...
class AWSResourceManager:
        ...
        try:
            match resource_type:
                case AWSResourceType.LAMBDA:
                    result = self.lambda_manager.create_or_update(
                        LambdaResourceSpec(**resource_spec)
                    )
                case AWSResourceType.IAM:
                    result = self.iam_manager.create_or_update(
                        IAMResourceSpec(**resource_spec)
                    )
                case AWSResourceType.SSM:
                    result = self.ssm_manager.create_or_update(
                        SSMResourceSpec(**resource_spec)
                    )
                case AWSResourceType.LOGS:
                    result = self.logs_manager.create_or_update(
                        LogsResourceSpec(**resource_spec)
                    )
                case AWSResourceType.TRIGGER:
                    result = self.trigger_manager.create_or_update(resource_spec)
                case _:
            ...
            self.logger.error(
                "Resource creation failed for %s: %s", resource_type, str(e)
            )
            return AWSResourceResponse(
    ...
    def read_resource(
        ...
        try:
            match resource_type:
                case AWSResourceType.LAMBDA:
                    result = self.lambda_manager.read(resource_id)
                case AWSResourceType.IAM:
                    result = self.iam_manager.read(
                        resource_id, kwargs.get("resource_subtype", "role")
                    )
                case AWSResourceType.SSM:
                    result = self.ssm_manager.read(resource_id)
                case AWSResourceType.LOGS:
                    result = self.logs_manager.read(resource_id)
                case AWSResourceType.TRIGGER:
                    result = self.trigger_manager.read(resource_id)
                case _:
                    assert_never(resource_type)
            ...
            self.logger.error(
                "Resource read failed for %s %s: %s", resource_type, resource_id, str(e)
            )
            return AWSResourceResponse(
    ...
    def delete_resource(
        ...
        try:
            match resource_type:
                case AWSResourceType.LAMBDA:
                    result = self.lambda_manager.delete(resource_id)
                case AWSResourceType.IAM:
                    result = self.iam_manager.delete(resource_id)
                case AWSResourceType.SSM:
                    result = self.ssm_manager.delete(resource_id)
                case AWSResourceType.LOGS:
                    result = self.logs_manager.delete(resource_id)
                case AWSResourceType.TRIGGER:
                    result = self.trigger_manager.delete(resource_id)
                case _:
                    assert_never(resource_type)
            ...
            self.logger.error(
                "Resource deletion failed for %s %s: %s",
                resource_type,
                resource_id,
                str(e),
            )
    ...
    def validate_aws_access(self) -> AWSResourceResponse:
        ...
        try:
            ...
            self.logger.info("AWS access validated for: %s", arn)
            return AWSResourceResponse(
                status="success", resource=dict(caller_identity), errors=[]
            )
        except (ClientError, NoCredentialsError, PartialCredentialsError) as e:
            self.logger.error("AWS access validation failed: %s", str(e))
            return AWSResourceResponse(
                status="error",
                resource=None,
