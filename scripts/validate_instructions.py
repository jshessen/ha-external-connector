...
def validate_transfer_block_consolidation() -> Tuple[bool, List[str]]:
    ...
    lambda_transfer_refs = find_transfer_block_references(lambda_content)
    ...
    has_detailed_transfer_content = len([ref for ref in aws_transfer_refs if "START" in ref or "END" in ref]) > 0
    ...
    if has_detailed_transfer_content:
        issues.append("⚠️  aws-patterns.md contains detailed transfer block content (should only reference)")
...
def validate_instruction_hierarchy() -> Tuple[bool, List[str]]:
    """Validate instruction directory hierarchy."""
    project_root = Path(__file__).parent.parent
    instructions_path = project_root / ".github" / "instructions"
...
def validate_cross_references() -> Tuple[bool, List[str]]:
    ...
    for md_file in md_files:
        ...
        internal_links[md_file] = [link[1] for link in links if not link[1].startswith('http')]
...
def validate_pattern_matching() -> Tuple[bool, List[str]]:
    ...
    if readme_path.exists():
        ...
        for file_pattern, patterns in expected_patterns.items():
            for pattern in patterns:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Validate GitHub Copilot instruction consistency")
    parser.add_argument(
        "--section",
        choices=["transfer-blocks", "hierarchy", "cross-refs", "patterns", "master", "all"],
        default="all",
