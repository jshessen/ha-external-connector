...
class AmazonSMAPIClient:
        ...
        try:
            async with self.session.post(
                ...
                if response.status != 200:
                    ...
                    self.logger.error(f"OAuth token exchange failed: {error_data}")
            ...
            self.logger.error(f"OAuth token exchange error: {e}")
    ...
    async def create_skill(
        ...
        try:
            ...
            self.logger.info(f"Successfully created Alexa skill: {skill_id}")
            return skill_id
        ...
        try:
            ...
            for _ in range(30):  # 5 minute timeout
                ...
                if status in ["SUCCEEDED", "FAILED"]:
                    ...
                    if status == "SUCCEEDED":
                        self.logger.info(f"Skill validation succeeded: {skill_id}")
                    else:
                        self.logger.warning(
                            f"Skill validation failed: {result.error_count} errors"
                        )
            ...
            self.logger.error(f"Skill validation error: {e}")
