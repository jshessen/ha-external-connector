...
def _handle_check_action() -> None:
    ...
    print(
        ...
        if venv_active and src_path.exists()
        else "⚠️  Status: Setup needed"
    )
...
def _validate_instruction_files(
    ...
    """Validate instruction files in a specific category."""
    category_path = instructions_path / category
    for file in files:
        file_path = category_path / file
        status = "✅" if file_path.exists() else "❌"
        print(f"  {status} {category}/{file}")


def _validate_transfer_blocks(specialized_path: Path) -> None:
    """Validate transfer block consistency between lambda and AWS patterns."""
...
def _handle_refresh_action() -> None:
    ...
    # File lists for different instruction categories
    ...
    # Validate all instruction categories
    for category, files in instruction_categories.items():
        _validate_instruction_files(instructions_path, category, files)

    # Validate transfer block consistency
    _validate_transfer_blocks(instructions_path / "specialized")
...
def _handle_validate_instructions_action() -> None:
    ...
    # Summary
    failed_checks: list[str] = [r for r in validation_results if r.startswith("❌")]
    if failed_checks:
